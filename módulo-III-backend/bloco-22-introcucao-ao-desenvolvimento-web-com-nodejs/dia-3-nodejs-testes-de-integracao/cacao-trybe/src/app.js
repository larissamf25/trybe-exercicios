...
app.get('/chocolates/total', async (req, res) => {
  const qnt = await cacaoTrybe.getAllChocolates();
  res.status(200).json({ totalChocolates: qnt.length });
});
...
app.put('/chocolates/:id', async (req, res) => {
  ...
  const updatedChocolate = await cacaoTrybe.updateChocolateById(Number(id), name, brandId);
  if(updatedChocolate) res.status(200).json({ chocolate: updatedChocolate });
